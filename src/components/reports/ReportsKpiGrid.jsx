...
export const ReportsKpiGrid = memo(function ReportsKpiGrid() {
  const { reportsKpis, isRefreshing, navigateTo } = useApp();

  const handleCardClick = useCallback((targetSubmodule) => {
    navigateTo('reports', targetSubmodule);
  }, [navigateTo]);
    ...
    return (
      <section className="kpi-grid-5">
        {Array.from({ length: 5 }).map((_, i) => (
          <div key={i} className="anodized-panel p-4" style={{ padding: '1rem' }} role="status" aria-label="Loading Reports KPI">
  ...
  const cards = Object.values(reportsKpis);

  return (
    <section className="kpi-grid-5" aria-label="Reports & Analytics Key Performance Indicators">
      {cards.map((card) => {
        const IconComp = ICON_MAP[card.icon] || TrendingUp;
        const isExpense = card.id === 'totalExpenses';
        ...
        return (
          ...
          >
            ...
            <div className="kpi-footer">
              ...
              <span className="kpi-trend" style={{ color: card.trendType === 'warning' ? '#f97316' : '#15803d' }}>
                {isExpense ? <ArrowDownRight size={13} aria-hidden="true" /> : <ArrowUpRight size={13} aria-hidden="true" />}
                {' '}{card.trend}
              </span>
            </div>
          </div>
