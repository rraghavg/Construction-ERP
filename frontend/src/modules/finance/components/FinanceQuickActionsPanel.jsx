...
import { FINANCE_QUICK_ACTIONS } from '../../../data/mockData';
import {
  TrendingUp,
  Receipt,
  FileText,
...
export const FinanceQuickActionsPanel = memo(function FinanceQuickActionsPanel({ onOpenIncomeModal }) {
  const { navigateTo } = useApp();

  const handleTileClick = useCallback((tile) => {
    if (tile.id === 'Add Income') {
      onOpenIncomeModal();
    } else {
      navigateTo('finance', tile.targetSubmodule);
    }
  }, [onOpenIncomeModal, navigateTo]);
  ...
  return (
    <div style={{ marginTop: '0.5rem' }}>
      ...
      <div className="section-tiles-grid" style={{ gridTemplateColumns: 'repeat(4, 1fr)' }}>
        {FINANCE_QUICK_ACTIONS.map((tile) => {
          const IconComp = ICON_MAP[tile.icon] || TrendingUp;
