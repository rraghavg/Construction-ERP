...
import { MAINTENANCE_QUICK_ACTIONS } from '../../../data/mockData';
import {
  Wrench,
  PlusCircle,
  UserPlus,
...
export const MaintenanceQuickActionsPanel = memo(function MaintenanceQuickActionsPanel({ onOpenAddModal }) {
  const { navigateTo } = useApp();

  const handleTileClick = useCallback((tile) => {
    if (tile.id === 'Add Complaint') {
      onOpenAddModal();
    } else {
      navigateTo('maintenance', tile.targetSubmodule);
    }
    ...
    <div style={{ marginTop: '0.5rem' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.85rem' }}>
        <h3 style={{ fontSize: '0.85rem', fontWeight: 800, textTransform: 'uppercase', display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          Maintenance Quick Actions Panel
        </h3>
      ...
      <div className="section-tiles-grid" style={{ gridTemplateColumns: 'repeat(6, 1fr)' }}>
        {MAINTENANCE_QUICK_ACTIONS.map((tile) => {
          const IconComp = ICON_MAP[tile.icon] || Wrench;
