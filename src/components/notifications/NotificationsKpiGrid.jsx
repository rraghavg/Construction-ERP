...
export const NotificationsKpiGrid = memo(function NotificationsKpiGrid() {
  const { notificationsKpis, isRefreshing, navigateTo } = useApp();

  const handleCardClick = useCallback((targetSubmodule) => {
    navigateTo('notifications', targetSubmodule);
  }, [navigateTo]);
    ...
    return (
      <section className="kpi-grid-5">
        {Array.from({ length: 5 }).map((_, i) => (
          <div key={i} className="anodized-panel p-4" style={{ padding: '1rem' }} role="status" aria-label="Loading Notifications KPI">
  ...
  const cards = Object.values(notificationsKpis);

  return (
    <section className="kpi-grid-5" aria-label="Notifications Centre Key Performance Indicators">
      {cards.map((card) => {
