...
export const AuditKpiGrid = memo(function AuditKpiGrid() {
  const { auditKpis, isRefreshing, navigateTo } = useApp();

  const handleCardClick = useCallback((targetSubmodule) => {
    navigateTo('audit-logs', targetSubmodule);
  }, [navigateTo]);
    ...
    return (
      <section className="kpi-grid-5">
        {Array.from({ length: 5 }).map((_, i) => (
          <div key={i} className="anodized-panel p-4" style={{ padding: '1rem' }} role="status" aria-label="Loading Audit KPI">
  ...
  const cards = Object.values(auditKpis);

  return (
    <section className="kpi-grid-5" aria-label="Audit & Compliance Key Performance Indicators">
      {cards.map((card) => {
        const IconComp = ICON_MAP[card.icon] || Activity;
        const isLowerBetter = card.id === 'failedLogins' || card.id === 'criticalActions';
          ...
          >
            ...
            <div className="kpi-footer">
              ...
              <span className="kpi-trend" style={{ color: '#15803d' }}>
                {isLowerBetter ? <ArrowDownRight size={13} aria-hidden="true" /> : <ArrowUpRight size={13} aria-hidden="true" />}
                {' '}{card.trend}
              </span>
            </div>
          </div>
