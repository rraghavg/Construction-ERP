...
export const CustomerKpiGrid = memo(function CustomerKpiGrid() {
  const { customerKpis, isRefreshing, navigateTo } = useApp();

  const handleCardClick = useCallback((targetSubmodule) => {
    navigateTo('customer-mgmt', targetSubmodule);
  }, [navigateTo]);
    ...
    return (
      <section className="kpi-grid-5">
        {Array.from({ length: 5 }).map((_, i) => (
          <div key={i} className="anodized-panel p-4" style={{ padding: '1rem' }} role="status" aria-label="Loading Customer KPI">
  ...
  const cards = Object.values(customerKpis);

  return (
    <section className="kpi-grid-5" aria-label="Customer Management Key Performance Indicators">
      {cards.map((card) => {
