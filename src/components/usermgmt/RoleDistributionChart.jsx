...
import { ROLE_DISTRIBUTION_DATA } from '../../data/mockData';
import { Doughnut } from 'react-chartjs-2';
import { PanelHeader } from '../shared/PanelHeader';
import { WidgetSkeleton } from '../shared/WidgetSkeleton';
import '../../utils/chartSetup';

export const RoleDistributionChart = memo(function RoleDistributionChart() {
  const { isRefreshing, navigateTo } = useApp();

  const { labels, values, colors, totalCount } = useMemo(() => {
    const raw = ROLE_DISTRIBUTION_DATA;
    return {
      labels: raw.segments.map((s) => s.role),
  ...
  const handleAction = useCallback(() => {
    navigateTo('user-mgmt', 'Roles & Hierarchy');
  }, [navigateTo]);
    ...
    <div className="data-grid-item" style={{ padding: '1.25rem' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', height: '220px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', flex: 1, overflowY: 'auto', maxHeight: '200px' }}>
          {ROLE_DISTRIBUTION_DATA.segments.map((seg) => (
            <div
              key={seg.role}
              tabIndex={0}
              role="button"
              ...
              aria-label={`Role ${seg.role}: ${seg.count} (${seg.pct}%)`}
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                <span
                  ...
                  aria-hidden="true"
                />
                <span style={{ fontWeight: 600 }}>{seg.role}</span>
              </div>
