...
export const MaintenanceKpiGrid = memo(function MaintenanceKpiGrid() {
  const { maintenanceKpis, isRefreshing, navigateTo } = useApp();

  const handleCardClick = useCallback((targetSubmodule) => {
    navigateTo('maintenance', targetSubmodule);
  }, [navigateTo]);
    ...
    return (
      <section className="kpi-grid-5">
        {Array.from({ length: 5 }).map((_, i) => (
          <div key={i} className="anodized-panel p-4" style={{ padding: '1rem' }} role="status" aria-label="Loading Maintenance KPI">
  ...
  const cards = Object.values(maintenanceKpis);

  return (
    <section className="kpi-grid-5" aria-label="Maintenance Management Key Performance Indicators">
      {cards.map((card) => {
        const IconComp = ICON_MAP[card.icon] || Wrench;
        const isTimeMetric = card.id === 'avgResolutionTime';
        ...
        return (
          ...
          >
            ...
            <div className="kpi-footer">
              ...
              <span className="kpi-trend" style={{ color: isTimeMetric ? '#15803d' : card.trendType === 'warning' ? '#f97316' : '#15803d' }}>
                {isTimeMetric ? <ArrowDownRight size={13} aria-hidden="true" /> : <ArrowUpRight size={13} aria-hidden="true" />}
                {' '}{card.trend}
              </span>
            </div>
          </div>
