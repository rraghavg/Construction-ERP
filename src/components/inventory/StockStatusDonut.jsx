...
import { STOCK_STATUS_SPLIT_DATA } from '../../data/mockData';
import { Doughnut } from 'react-chartjs-2';
import { PanelHeader } from '../shared/PanelHeader';
import { WidgetSkeleton } from '../shared/WidgetSkeleton';
import '../../utils/chartSetup';

export const StockStatusDonut = memo(function StockStatusDonut() {
  const { isRefreshing, navigateTo } = useApp();

  const { labels, values, colors, totalCount } = useMemo(() => {
    const raw = STOCK_STATUS_SPLIT_DATA;
    return {
  ...
  const options = useMemo(() => ({
    ...
    plugins: {
      ...
      tooltip: {
        callbacks: {
          label: (context) => ` ${context.label}: ${context.parsed} SKUs`
        }
      }
    }
  }), []);
    ...
    <div className="data-grid-item" style={{ padding: '1.25rem' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', height: '220px' }}>
        ...
        <div style={{ position: 'relative', width: '140px', height: '140px', flexShrink: 0 }}>
          ...
          >
            <span className="mono-data" style={{ fontSize: '1.2rem', fontWeight: 800, color: '#f97316' }}>
              23 Low
            </span>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '5px', flex: 1 }}>
          {STOCK_STATUS_SPLIT_DATA.segments.map((seg) => (
            <div
              key={seg.label}
              tabIndex={0}
              role="button"
