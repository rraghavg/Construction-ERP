...
export const HrKpiGrid = memo(function HrKpiGrid() {
  const { hrKpis, isRefreshing, navigateTo } = useApp();

  const handleCardClick = useCallback((targetSubmodule) => {
    navigateTo('hr', targetSubmodule);
  }, [navigateTo]);
    ...
    return (
      <section className="kpi-grid-5">
        {Array.from({ length: 5 }).map((_, i) => (
          <div key={i} className="anodized-panel p-4" style={{ padding: '1rem' }} role="status" aria-label="Loading HR KPI">
  ...
  const cards = Object.values(hrKpis);

  return (
    <section className="kpi-grid-5" aria-label="HR Management Key Performance Indicators">
      {cards.map((card) => {
